...
void SharpenBrush::setColorWithFilter(const Point source)
{
	ImpressionistDoc* pDoc = GetDocument();
	...
	color[0] = pDoc->sharpenFilter->filterPixel(pDoc, 0, source, pDoc->m_nWidth, pDoc->m_nHeight);
	color[1] = pDoc->sharpenFilter->filterPixel(pDoc, 1, source, pDoc->m_nWidth, pDoc->m_nHeight);
	color[2] = pDoc->sharpenFilter->filterPixel(pDoc, 2, source, pDoc->m_nWidth, pDoc->m_nHeight);
